...
@dataclass
class LLama2Config:
    """Configuration for LLama2 model"""
...
class LLama2DecoderLayer(nn.Module):
    """Decoder layer for LLama2"""
    def __init__(self, config: LLama2Config):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LLama2Attention(config=config)
...
class LLama2Model(nn.Module):
    ...
    def __init__(self, config: LLama2Config):
        ...
        self.layers = nn.ModuleList([LLama2DecoderLayer(config) for _ in range(config.num_hidden_layers)])
...
def create_llama2_1_7b() -> LLama2ForCausalLM:
    """Create a LLama2 model with 1.7B parameters"""
    config = LLama2Config(
        vocab_size=32000,
        hidden_size=2048,
    ...
    return LLama2ForCausalLM(config) 